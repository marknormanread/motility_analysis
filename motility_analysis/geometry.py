...
def normalise_vector(u):
    """ Returns the normalised unit vector of u, returning each of the three components as a tuple. """
    x, y, z = u
...
def signed_angle_between_vectors(v, w, up):
    """ Returns angle between v and w relative to 'up'. Return value ranges from -180 to 180 degrees. """
    ...
    len_n = vector_length(cross)
    if len_n == 0.0:
        raise InvalidAngleException
    dot = dot_product(v, w)
    radians = math.atan2(len_n, dot)
    if (dot_product(up, cross) < 0):
        radians = -radians
